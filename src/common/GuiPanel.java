...
//	private String goalname;
	...
	private AbstractTableModel items = new AbstractTableModel()
	{
		...
		public String getColumnName(int column)
		{
			...
			case 0:
				return "title";
			case 1:
				return "Check Request";//if data within the dataset and can be sold out
			case 2:
		...
		public Object getValueAt(int row, int column)
		{
			...
			{
				value = new String(order.getCheckRequest());
				//System.out.print(value);
			}
			else if(column == 2)
			{
				value = new Integer (order.getTransformationcode());
				//System.out.print(value);
			}
			else if(column == 3)
			{
	...
	public GuiPanel(final IExternalAccess agent)//, final boolean buy)
	{
		...
		this.agent = agent;
		final boolean buy = isProcessor(agent);
		
		if(buy)
		{
			itemlabel = " Data to Process ";
//			goalname = "purchase_";
			addorderlabel = "Add new collect order";
		}
		else
		{
			itemlabel = " Data to Collect ";
//			goalname = "sell_";
			addorderlabel = "Add new process order";
		}
...
//		});
							...
							public void customResultAvailable(Object result)
							{
								...
								while(dia.requestInput(cs.getTime()))
								{
									try
									{
										...
										Date deadline = dformat.parse(dia.deadline.getText());//String Req = dia.Req.getText();
										String CheckRequest = dia.CheckRequest.getText();
										Integer Transformationcode = Integer.parseInt(dia.Transformationcode.getText());
										
										//String Req = dia.Req.getText();
										final Order order = new Order(title, deadline, CheckRequest,Transformationcode, buy, cs);
	...
	//									});
									...
									{
										JOptionPane.showMessageDialog(GuiPanel.this, "According to the GDPR it is not permitted.", "Input error", JOptionPane.ERROR_MESSAGE);
									}
...
//					});
							...
							.addResultListener(new SwingDefaultResultListener(GuiPanel.this)
						{
							public void customResultAvailable(Object result)
							{
								...
								if(row >= 0 && row < orders.size())
								{
									...
									edit_dialog.Transformationcode.setText(Integer.toString(order.getTransformationcode()));
									edit_dialog.CheckRequest.setText(order.getCheckRequest());
									...
									while(edit_dialog.requestInput(cs.getTime()))
									{
										try
										{
											...
											Integer Transformationcode = Integer.parseInt( edit_dialog.Transformationcode.getText());
											String CheckRequest = edit_dialog.CheckRequest.getText();
											Date deadline = dformat.parse(edit_dialog.deadline.getText());
											order.settitle(title);
											order.setTransformationcode(Transformationcode);
											order.setCheckRequest(CheckRequest);
	...
	//										});
										...
										{
											JOptionPane.showMessageDialog(GuiPanel.this, "It is not according to the GDPR.", "Input error", JOptionPane.ERROR_MESSAGE);
										}
										catch(ParseException e1)
										{
											JOptionPane.showMessageDialog(GuiPanel.this, "Wrong date format, use YYYY/MM/DD hh:mm.", "Input error", JOptionPane.ERROR_MESSAGE);
...
//			});
	...
	//	@SuppressWarnings("unchecked")
		...
		private JTextField CheckRequest = new JTextField(20);
		private JTextField Transformationcode = new JTextField(20);
						...
						public void resultAvailable(IClockService clock)
						{
							try
							{
								if(buy)
								{
									...
									orders.addItem(new Order("Data engin1", null, "Permitted", 1, buy, clock));
									orders.addItem(new Order("Data engin2", null,"Permitted", 2, buy, clock));
									orders.addItem(new Order("Customer names", null,"notPermitted", 3, buy, clock));
									orders.addItem(new Order("Pilot names", null, "notPermitted", 4, buy, clock));
								}
								else
								{
									orders.addItem(new Order("Data engin1", null, "Permitted",1, buy, clock));
							...
							leftcons.gridy = rightcons.gridy = row++;
							label = new JLabel("Check Request");
							label.setMinimumSize(labeldim);
							label.setPreferredSize(labeldim);
							center.add(label, leftcons);
							center.add(CheckRequest, rightcons);
	
							leftcons.gridy = rightcons.gridy = row++;
							label = new JLabel("Transformation code");
							label.setMinimumSize(labeldim);
							label.setPreferredSize(labeldim);
							center.add(label, leftcons);
							center.add(Transformationcode, rightcons);
							...
							orders.addActionListener(new ActionListener()
							{
								public void actionPerformed(ActionEvent e)
								{
									...
									Transformationcode.setText("" + order.getTransformationcode());
									CheckRequest.setText("" + order.getCheckRequest());
								}
							});
						}
	...
	/**
	 *  Test if agent is a processor.
	 */
	public static boolean isProcessor(IExternalAccess agent)
	{
		return agent.getModel().getName().indexOf("Processor")!=-1;
	}
}
...
class NegotiationDetailsDialog extends JDialog
{
	 ...
	 * 
	 */
//	private static final long serialVersionUID = 5824450065151482636L;
